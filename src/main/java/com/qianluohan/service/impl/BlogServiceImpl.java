...
import com.qianluohan.dao.BlogRespsitory;
...
public class BlogServiceImpl implements BlogService {
    ...
    @Autowired
    private BlogRespsitory blogRespsitory;

    @Override
    public Blog getBlog(Long id) {
        return blogRespsitory.findOne(id);
    }
    ...
    public Blog updateBlog(Long id, Blog blog) {
        Blog b = blogRespsitory.findOne(id);
        if(b == null){
            throw new NotFoundException("该博客不存在");
        }
        BeanUtils.copyProperties(b, blog);
        return blogRespsitory.save(b);
    }

    @Override
    public void deleteBlog(Long id) {
        blogRespsitory.delete(id);
    }
}
